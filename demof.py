...
# ✅ [추가] UPPER merge point 선택용: 점-폴리라인 최소거리^2
...
class UpperRightCollisionGuardianSplitDZ(Node):
    def __init__(self):
        ...
        # 파라미터
        self.NEAR_TOL_RIGHT, self.SAFETY_DIST_RIGHT = 0.55, 1.5
        self.NEAR_TOL_UPPER, self.SAFETY_DIST_UPPER = 1.0, 2.0
        self.UPPER_RELEASE_DIST = 2.20
        self.NEAR_TOL_ROTARY = 0.50

        self.ACCEL_YIELD, self.ACCEL_RESUME = 0.01, TARGET_VELOCITY
        self.MIN_ACCEL, self.TICK, self.RAMP_PER_SEC = 0.0, 0.05, 0.35

        # ✅ [추가] UPPER yield tie 정책(원래 의도)
        self.UPPER_TIE_EPS = 0.10
        self.UPPER_TIE_YIELD_ID = 1  # tie면 1이 양보
        ...
        # ====================================================
        # ✅ [수정] Upper Merge Point: centroid(평균) → "두 경로에 동시에 가장 가까운 점"
        # ====================================================
    ...
    def tick(self):
        ...
        if self.upper_merge:
            ...
            if self.upper_lock_yield_id:
                ...
                # ✅ [수정] "더 먼 차량이 양보" + tie 처리(대칭)
                if rem1 < rem2 - self.UPPER_TIE_EPS:
                    yield_id = 2
                elif rem2 < rem1 - self.UPPER_TIE_EPS:
                    yield_id = 1
                else:
                    yield_id = self.UPPER_TIE_YIELD_ID  # tie면 1이 양보
