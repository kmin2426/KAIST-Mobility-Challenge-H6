...
class MapPredictionDriver(Node):
    ...
    def drive_loop(self):
        ...
        # lap counting
        # ===========================
        if self.prev_idx_for_lap is not None:
            N = len(self.path_pts)

            # 뒤에서 앞쪽으로 넘어가면 1 lap
            if self.prev_idx_for_lap > int(0.8 * N) and curr_idx < int(0.2 * N):
                self.lap_count += 1
                self.get_logger().info(
                    f"🏁 [Car{self.vid}] Lap completed! Total laps = {self.lap_count}"
                )

        self.prev_idx_for_lap = curr_idx

        round_trip = self.lap_count
        self.get_logger().info(
            f"🔁 [Car{self.vid}] RoundTrips = {round_trip} (laps={self.lap_count})"
        )
...
def main(args=None):
    ...
    drivers = [
        MapPredictionDriver(
            i,
            os.path.join(PATH_DIR, f"path3_{i}.json")
        )
        for i in range(1, 5)
    ]
