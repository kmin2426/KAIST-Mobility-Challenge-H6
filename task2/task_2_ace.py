...
class MergeCorridorController(Node):
    # --- Tuning Constants ---
    MERGE_ZONE = {'x_min': 3.80, 'x_max': 5.31, 'y_min': -2.66, 'y_max': 0.77}
    ...
    def __init__(self):
        ...
        # --- Path Loading ---
        self.paths = {
            1: self.load_path("tool/path/lane1.json"),
            2: self.load_path("tool/path/lane2.json"),
            3: self.load_path("tool/path/lane3.json")
        }
        self.path_arrs = {k: np.column_stack((v['X'], v['Y'])) if v['X'] else np.empty((0, 2)) for k, v in self.paths.items()}

        # --- HV Configuration ---
        self.LANE_HV_MAP = {
            1: list(range(19, 23)),  # Slow
            2: list(range(23, 31)),  # Fast
            3: list(range(31, 37))   # Static
        }
    ...
    def check_side_safety(self, target_lane):
        """
        [Gatekeeper] Blocks LC if ANY car is physically overlapping in the blind spot.
        Zone: Rear 0.5m ~ Front 0.5m | Side 0.3m
        """
    ...
    def get_lane_info(self, target_lane):
        """ Calculates Min Gaps & Speeds. Includes Special Merge Logic. """
        min_f, f_v = 999.0, 0.0
        min_r, r_v = 999.0, 0.0
        ...
        for hv_id in ids:
            ...
            # Rear Car
                ...
                # [Merge Protection] If Lane 2 car is too close (side/rear), inflate COST
                check_merge_rear = self.is_in_merge_zone() and (target_lane == 2)
                if check_merge_rear and gap < 0.5:
                    if gap < min_f: # Treat as 'Wall' in front to spike cost
                        min_f = 0.01 
                        f_v = d['v']
                    continue
    ...
    def should_yield(self):
        ...
        # If check_side_safety(2) is False -> It's dangerous -> Yield(True)
