...
# # FSM Parameters
# FRONT_CLOSE   = 1.1   # trigger: front too close
# BACK_CLOSE    = 0.8   # trigger: back too close
# BACK_FAST_DV  = 0.3   # trigger: back approaching fast (dv >)
# COOLDOWN_SEC  = 2.0
...
#                 # circular ROI filter (ignore too far vehicles)
#                 dx = hv.x - cav_x
#                 dy = hv.y - cav_y
#                 if (dx * dx + dy * dy) > r2:
#                     continue
...
#                 # IMPORTANT: ds must be computed on the SAME lane axis
#                 ds = hv.s - cav_s_lane
...
#     # helper: find nearest front/back in a given lane
...
#     # helper: safety check for candidate lane
#     def lane_is_safe(self, lane: int, cav_s: float) -> bool:
#         fid, fds, bid, bds = self.get_front_back_in_lane(lane, cav_s)

#         # no car in front within SAFE_FRONT_GAP -> ok
#         front_ok = (fid is None) or (fds >= SAFE_FRONT_GAP)

#         # no car behind within SAFE_BACK_GAP -> ok (remember bds is negative)
#         back_ok = (bid is None) or (abs(bds) >= SAFE_BACK_GAP)

#         # optionally: if behind is fast approaching, treat as unsafe
#         if bid is not None and bid in self.hv_v:
#             dv = self.hv_v[bid] - self.ego_v
#             if dv > BACK_FAST_DV:
#                 back_ok = False
...
#         # ego speed
#         if self.prev_cav_s is not None:
#             self.ego_v = (cav_s - self.prev_cav_s) / dt
#         self.prev_cav_s = cav_s
#         self.prev_time = now

#         # track only currently relevant HVs
#         for hid in [front_id, back_id]:
#             if hid is None:
#                 continue
#             hv = self.proj.hvs.get(hid)
#             if hv is None or not hv.valid:
#                 continue
#             prev = self.prev_hv_s.get(hid)
#             if prev is not None:
#                 self.hv_v[hid] = (hv.s - prev) / dt
#             self.prev_hv_s[hid] = hv.s

#     # FSM
#     def fsm_step(self, cav_lane: int, cav_s: float,
#                  front_id: Optional[int], front_ds: float,
#                  back_id: Optional[int], back_ds: float):
#         now = self.get_clock().now().nanoseconds * 1e-9
#         in_cooldown = (now - self.last_lc_time) < self.cooldown

#         front_close = (front_id is not None) and (0.0 < front_ds < FRONT_CLOSE)
#         back_close = (back_id is not None) and (-BACK_CLOSE < back_ds < 0.0)

#         back_fast = False
#         if back_id is not None and back_id in self.hv_v:
#             back_fast = (self.hv_v[back_id] - self.ego_v) > BACK_FAST_DV

#         need_prepare = front_close or back_close or back_fast

#         if self.state == LCState.KEEP_LANE:
#             self.target_lane = cav_lane
#             if (not in_cooldown) and need_prepare:
...
#             if cav_lane == 1:
#                 neighbor_order = [2, 0]   # 2 우선
#             else:
#                 neighbor_order = [cav_lane - 1, cav_lane + 1]  # 기존(필요시 유지)

#             candidates = []
#             for nl in neighbor_order:
#                 if nl in [0, 1, 2] and self.lane_is_safe(nl, cav_s):
#                     candidates.append(nl)

#             if candidates:
#                 # TODO: 우선순위 규칙(왼쪽 우선/오른쪽 우선/베스트 등) 여기서 결정
#                 self.target_lane = candidates[0]
...
#             # 완료 조건(간단): lane이 target으로 바뀌면 완료
#             if self.target_lane is not None and cav_lane == self.target_lane:
#                 self.state = LCState.KEEP_LANE
#                 self.target_lane = cav_lane
...
#             self.get_logger().info('CAV pose not received yet.')
#             return

#         cav_x = self.proj.ego.x
#         cav_y = self.proj.ego.y

#         # ego projection (best lane by abs_d) + store cav_s for each lane
#         best_lane = None
#         best_abs_d = INF
#         best_tuple = None
...
#                 best_abs_d = abs_d
#                 best_lane = lane
#                 best_tuple = (s, d, abs_d, mode)

#         cav_lane = best_lane
#         cav_s, cav_d, cav_abs_d, cav_mode = best_tuple

#         self.proj.ego.lane = cav_lane
#         self.proj.ego.s = cav_s
#         self.proj.ego.d = cav_d
#         self.proj.ego.abs_d = cav_abs_d

#         # HV projection for ALL HVs (assign lane/s/d/abs_d)
#         for hv_id, hv in self.proj.hvs.items():
#             if not hv.valid:
#                 continue
...
#             for lane in ALLOWED_LANES:
#                 s, d, i, u, abs_d, qx, qy, mode = self.proj.robust_project(hv, lane, hv.x, hv.y)
#                 if abs_d < hv_best_abs_d:
#                     hv_best_abs_d = abs_d
#                     hv_best_lane = lane
#                     hv_best_s, hv_best_d = s, d

#             hv.lane = hv_best_lane
#             hv.s = hv_best_s
#             hv.d = hv_best_d
#             hv.abs_d = hv_best_abs_d

#         # build lane-wise front/back table (uses projection results + circular ROI)
#         self.build_lane_front_back(cav_x, cav_y)

#         # current lane front/back
#         front_id, front_ds, back_id, back_ds = self.get_front_back_in_lane(cav_lane, cav_s)

#         # FSM
#         self.update_speed_estimates(cav_s, front_id, back_id)
#         self.fsm_step(cav_lane, cav_s, front_id, front_ds, back_id, back_ds)
...
#         # Publish target lane and behavior
#         msg_lane = Int32()
