...
class VehicleController(Node):
    def __init__(
        ...
        self.steer_f = 0.0
        self.STEER_ALPHA = 0.88
        self.STEER_RATE_MAX = 2.0

        # ===== [REAL CAR PATCH] yaw deadband =====
        self.YAW_DEADBAND = 0.02

        # ===== [REAL CAR PATCH] dt-based ramp =====
    ...
    def _callback_pose(self, msg):
        ...
        raw_yaw = float(msg.pose.orientation.z)  # yaw hack
    ...
    def _control_vehicle(self, target_vel):
        ...
        dt = (now - self.last_ctrl_time).nanoseconds / 1e9
        self.last_ctrl_time = now
        dt = max(0.01, min(0.2, dt))


        ACCEL_UP = 0.6   # m/s^2 (실차용)
        ACCEL_DN = 1.2   # m/s^2

        if target_vel > self.current_cmd_vel:
            self.current_cmd_vel = min(target_vel, self.current_cmd_vel + ACCEL_UP * dt)
        else:
            self.current_cmd_vel = max(target_vel, self.current_cmd_vel - ACCEL_DN * dt)
        ...
        d_raw = (yaw_err - self.prev_error) / dt
        self.d_f = lpf(self.d_f, d_raw, self.D_ALPHA)
        d = CTRL_PARAMS["kd"] * max(-self.D_MAX, min(self.D_MAX, self.d_f))
        ...
        # LPF + rate limit
        steer = lpf(self.steer_f, steer, self.STEER_ALPHA)
        max_step = self.STEER_RATE_MAX * dt
        steer = max(self.steer_f - max_step, min(self.steer_f + max_step, steer))
        self.steer_f = steer

        self.prev_error = yaw_err

        if abs(steer) > 0.7:
            self.current_cmd_vel = min(self.current_cmd_vel, 0.45)
        elif abs(steer) > 0.5:
            self.current_cmd_vel = min(self.current_cmd_vel, 0.55)
