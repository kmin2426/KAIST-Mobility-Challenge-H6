...
# projection.py의 PATH_FILES 순서 기준
# - path_hv_2.csv  -> lane 0
# - path_hv_3.csv  -> lane 1
LANE_HV_ZONE_2 = 0   # path_hv_2.csv
LANE_HV_ZONE_3 = 1   # path_hv_3.csv

# CAV별 "자기 zone lane" (너네 projection.py PATH_FILES에 맞춰둔 인덱스 그대로)
LANE_CAV1_ZONE = 3
LANE_CAV2_ZONE = 4
LANE_CAV3_ZONE = 5
LANE_CAV4_ZONE = 6
...
# CAV별 HV 트리거 존 lane
# - CAV 2,3 -> path_hv_2.csv (lane 0)
# - CAV 1,4 -> path_hv_3.csv (lane 1)
...
class HVTailgateNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            "[hv.py] Tailgating gate: (CAV in own zone) -> (HV trig IN이면 FREE, 아니면 STOP)\n"
            f"  HV_ZONE_ABS_D={HV_ZONE_ABS_D:.2f} | CAV_ZONE_ABS_D={CAV_ZONE_ABS_D:.2f}\n"
            f"  MIN_SPEED={MIN_SPEED:.2f} | FREE_SPEED={FREE_SPEED:.2f}\n"
            f"  lane{LANE_HV_ZONE_2}(path_hv_2.csv) HV_ZONE_S={HV_ZONE_S[LANE_HV_ZONE_2]}\n"
            f"  lane{LANE_HV_ZONE_3}(path_hv_3.csv) HV_ZONE_S={HV_ZONE_S[LANE_HV_ZONE_3]}\n"
            f"  TRIG: CAV2,3->lane{LANE_HV_ZONE_2} | CAV1,4->lane{LANE_HV_ZONE_3}"
        )
    ...
    def on_timer(self):
        ...
        lines.append(
            f"HV_ZONE lane{LANE_HV_ZONE_2}(hv_2): {'IN ' if hv_in_zone2 else 'OUT'} "
            f"| S={HV_ZONE_S[LANE_HV_ZONE_2]} | by {zone2_who}"
        )
        lines.append(
            f"HV_ZONE lane{LANE_HV_ZONE_3}(hv_3): {'IN ' if hv_in_zone3 else 'OUT'} "
            f"| S={HV_ZONE_S[LANE_HV_ZONE_3]} | by {zone3_who}"
        )
        lines.append("---")

        # CAV 처리: "자기 zone 안"일 때만 gate 적용 (CAV1이랑 2,3,4도 완전히 동일)
        for vid in CAV_IDS:
            ...
            # zone 안이면: 트리거 lane의 HV 존이 IN일 때만 FREE, 아니면 STOP
