...
class AllZonesGuardianMux(Node):
    def __init__(self):
        ...
        self.RADIUS = 1.6333
        self.CORE_RADIUS = 0.9
        self.EXIT_RADIUS = 1.05
        ...
        # ==========================
        # [NEW] intersection 진입 시각 기록
        # ==========================
        self.enter_time = {1: None, 2: None}
        self.SIMULTANEOUS_DT = 0.5  # [sec] ← "거의 동시에" 기준
    ...
    def _intersection_limits(self):
        ...
        if dist_c1 < self.RADIUS and self.enter_time[1] is None:
             self.enter_time[1] = now_t
             print(f"[INTERSECTION] CAV1 entered radius at t={now_t:.2f}s")
        if dist_c2 < self.RADIUS and self.enter_time[2] is None:
             self.enter_time[2] = now_t
             print(f"[INTERSECTION] CAV2 entered radius at t={now_t:.2f}s")

        # leader 선택
        if self.leader_id is None and not self.intersection_lock:
            if self.enter_time[1] is not None and self.enter_time[2] is not None:
                dt = abs(self.enter_time[1] - self.enter_time[2])
                if dt < self.SIMULTANEOUS_DT:
                     self.leader_id = 1
                     print(f"🟡 [INTERSECTION TIE] " f"simultaneous entry detected (dt={dt:.2f}s) → priority=CAV1")
                else : 
                     self.leader_id = 1 if self.enter_time[1] < self.enter_time[2] else 2
                     print( f"🏁 [INTERSECTION] " f"priority=CAV{self.leader_id} " f"(t1={self.enter_time[1]:.2f}, t2={self.enter_time[2]:.2f})" )
                self.exit_counter=0
